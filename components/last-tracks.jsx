...
export default function LastTracks () {
  ...
  useEffect(() => {
    getLastTracks().then((last_tracks)=> setTracks(last_tracks.props.search)) 
  }, [])
  ...
  return (
    <>
      ...
      <div className="grid relative mb-16 rounded-md text-center backdrop-blur-2xl xs:grid-cols-2 sm:grid-cols-3 sm:gap-6 md:grid-cols-4 md:gap-6 lg:mb-16 lg:grid-cols-5 lg:gap-6 lg:text-left xl:grid-cols-6 xl:gap-8 xs:max-sm:gap-6 xs:max-sm:p-4 xs:max-sm:-mt-20 xs:max-sm:mb-12">
        {tracks?.map((item) => {
          return (
            <>
