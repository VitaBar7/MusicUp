    ...
    return (
        ...
        <main className="flex min-h-screen flex-col items-center justify-between p-24">
            <div className="mt-6 mb-32 max-h-fit grid text-center lg:mb-0 lg:grid-cols-1 lg:text-left">
                <div className="group rounded-sm border border-transparent mx-1 px-3 py-2 transition-colors hover:border-gray-300 hover:bg-dirty-white hover:text-dark-green">
                    <img
                        ...
                        />
                    <h2 className="mb-1 text-2xl  tracking-wider hover:text-dark-green">
                        {trackDetails?.name} 
                    </h2>
                    <p className= "m-0 mb-2 max-w-5xl hover:text-dark-green text-md opacity-70">
                        album: {trackDetails?.album?.name} 
                    </p>
                    <a href={trackDetails?.artists[0]?.external_urls.spotify} target="_blank" rel="noopener noreferrer">
                        <p className= "m-0 mb-2 max-w-5xl text-xl hover:text-dark-green tracking-wide opacity-100 hover:text-normal">
                           by {trackDetails?.artists[0]?.name} 
                        </p>
                    </a>

                    <a href={trackDetails?.external_urls.spotify} className="inline-flex items-center m-auto px-3 py-2 text-sm font-medium text-center text-white bg-gray-700 rounded-lg hover:bg-dark-green focus:ring-2 focus:outline-none focus:ring-pink-700 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" target="_blank" rel="noopener noreferrer">
                        play in spotify
